...
const Sidebar = ({ vistaActiva, setVistaActiva, user, puedeVerDashboard, sidebarCollapsed, setSidebarCollapsed }) => {
  // Verificar si el usuario puede hacer solicitudes (solo nivel 1)
  const puedeHacerSolicitudes = user && (
    user.isSupremeBoss || 
    (user.cargoId === 4 && user.campaniaId === 5) || 
    ['002702515', '76157106', '46142691'].includes(user.dni)
  );

  const menuItems = [
    {
      id: 'dashboard',
      label: user?.isSupremeBoss ? 'Dashboard General' : (puedeVerDashboard ? 'Mi Equipo' : 'Mis Tareas'),
      icon: (
    ...
    {
      id: 'calendario',
      label: user?.isSupremeBoss ? 'Calendario General' : 'Mi Calendario',
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  // Agregar solicitudes solo si el usuario puede hacer solicitudes
  if (puedeHacerSolicitudes) {
    menuItems.push({
      id: 'solicitudes',
      label: 'Solicitudes',
      icon: (
  ...
  return (
    <div className={`${sidebarCollapsed ? 'w-16' : 'w-64'} bg-white shadow-lg border-r border-gray-200 h-screen fixed left-0 top-0 z-40 transition-all duration-300 ease-in-out`}>
      {/* Logo y título */}
      <div className={`${sidebarCollapsed ? 'p-2' : 'p-6'} border-b border-gray-200`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 620.79 618.96" className={`${sidebarCollapsed ? 'h-6 w-auto mx-auto' : 'h-8 w-auto mr-3'}`}>
              <defs>
      ...
      <div className={`${sidebarCollapsed ? 'p-2' : 'p-4'} border-b border-gray-200`}>
        <div className="flex items-center">
          <div className={`${sidebarCollapsed ? 'w-8 h-8 mx-auto' : 'w-10 h-10'} bg-gradient-to-br from-blue-500 to-purple-600 rounded-full flex items-center justify-center text-white font-bold text-sm`}>
        ...
        <div className="space-y-1">
          {menuItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center ${sidebarCollapsed ? 'justify-center px-2 py-3' : 'px-4 py-3'} text-sm font-medium rounded-lg mx-2 transition-colors ${
                vistaActiva === item.id
            ...
            >
              <span className={`${sidebarCollapsed ? '' : 'mr-3'} ${vistaActiva === item.id ? 'text-blue-700' : 'text-gray-400'}`}>
                {item.icon}
              </span>
      ...
      <div className={`absolute bottom-0 left-0 right-0 ${sidebarCollapsed ? 'p-2' : 'p-4'} border-t border-gray-200`}>
